...
- `AgentSkillCatalog` 在启动时只解析技能名称、描述和声明工具等轻量元数据；代码生成阶段加载 `java-code-generation` 与 `security-review`，测试失败后才加载 `test-failure-repair` 与 `security-review` 的正文，减少无关上下文并保留激活轨迹。
