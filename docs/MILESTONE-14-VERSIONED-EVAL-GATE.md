...
> 我把模型评测从一次性页面结果升级成版本化 Eval Run。每次运行会记录模型、Prompt、数据集逻辑版本和数据集 SHA-256，并持久化完整指标和逐题结果。发布门禁既检查 Coverage、Accuracy、Macro Recall 和最低分类 Recall 的绝对阈值，也与最近一次通过基线比较；超过回退预算就拒绝发布。Mock 和模型降级结果标记为 NOT_EVALUATED，不会污染基线。正式运行使用 POST，因为它会消耗额度并写入评测记录，历史查询才使用 GET。

## 简历可写版本

> 构建版本化 Agent Evals 与发布门禁，为每次 Claim–Evidence 评测记录模型、Prompt、数据集版本及 SHA-256 指纹，通过原子文件 Checkpoint 持久化完整指标与逐题结果；结合绝对质量阈值和最近通过基线检测效果回退，并将 Mock/降级结果隔离为未评估状态，避免污染发布基线。
